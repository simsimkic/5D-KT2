...
{
    public class AccommodationOwnerRateService
    {
        private static AccommodationOwnerRateService _instance;
        private readonly AccommodationOwnerRateRepository _repository;
        private List<AccommodationOwnerRate> _rates;
        private readonly FileManager _fileManager;

        public static AccommodationOwnerRateService GetInstance()
        {
            if (_instance == null)
            {
                _instance = new AccommodationOwnerRateService();
            }

            return _instance;
        }

        private AccommodationOwnerRateService()
        {
            _repository = new AccommodationOwnerRateRepository();
            _fileManager = new FileManager();
            _rates = _repository.Load();
            LoadAccommodationOwnerRates();
        }
        ...
        public List<AccommodationOwnerRate> GetReviewsByAccommodationId(int accommodationId)
        {
            return _rates.FindAll(r => r.AccommodationReservation.AccomodationId == accommodationId);
        }
    }
}
